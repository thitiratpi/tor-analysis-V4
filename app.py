...
<style>
    ...
    /* ===== GLOBAL TYPOGRAPHY ===== */
    .stApp { background-color: white; color: #334155; font-family: 'Segoe UI', sans-serif; }
    h1, h2, h3, h4, h5 { color: #1E293B !important; font-weight: 700 !important; letter-spacing: -0.5px; }
    .main-header {
    ...
    .file-info-card {
        background-color: #F8FAFC; padding: 1.25rem; border-radius: 12px;
        border: 1px solid #E2E8F0; height: 100%; display: flex; flex-direction: column; justify-content: center;
    }

    /* ===== STATS ===== */
    .stat-label { font-size: 0.9rem; text-transform: uppercase; letter-spacing: 1px; color: #64748B; font-weight: 700; margin-bottom: 1.5rem; text-align: center; }
    .stat-value-big { font-size: 2.0rem; font-weight: 800; color: #2563EB; line-height: 1; }
    .stat-row { display: flex; justify-content: space-between; align-items: center; padding: 12px 10px; border-bottom: 1px solid #F1F5F9; }
    .stat-row:last-child { border-bottom: none; }
    .stat-name { font-size: 1.05rem; color: #334155; font-weight: 500; }
    .stat-count { font-size: 1.2rem; font-weight: 700; color: #2563EB; }

    /* ===== BUTTONS ===== */
    .stButton > button { width: 100%; border-radius: 8px; height: 3.2rem; font-weight: 600; border: 1px solid #E2E8F0; background-color: white; color: #1E293B; transition: all 0.2s; }
    .stButton > button:hover { border-color: #2563EB; color: #2563EB; background-color: #EFF6FF; transform: translateY(-2px); box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    
    div[data-testid="stForm"] button[kind="secondary"] {
        background: linear-gradient(135deg, #10B981 0%, #059669 100%); color: white; border: none; margin-top: 10px; box-shadow: 0 4px 6px rgba(16, 185, 129, 0.3);
    }
    div[data-testid="stForm"] button[kind="secondary"]:hover { box-shadow: 0 6px 10px rgba(16, 185, 129, 0.4); color: white; }
    div[data-testid="stButton"] button[kind="primary"] { background: linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%); color: white; border: none; }

    /* ===== UPLOAD AREA ===== */
    [data-testid="stFileUploader"] { background-color: #F8FAFC; border: 2px dashed #CBD5E1; border-radius: 12px; padding: 2rem; }
...
with st.sidebar:
    ...
    try:
        ...
        st.markdown("""<div style='background-color: #DCFCE7; padding: 12px; border-radius: 8px; border-left: 4px solid #10B981; margin-bottom: 1rem;'><p style='margin: 0; color: #065F46; font-weight: 600; font-size: 0.9rem;'>✅ Gemini AI: Connected</p></div>""", unsafe_allow_html=True)
    except Exception as e:
        st.markdown("""<div style='background-color: #FEE2E2; padding: 12px; border-radius: 8px; border-left: 4px solid #EF4444; margin-bottom: 1rem;'><p style='margin: 0; color: #991B1B; font-weight: 600; font-size: 0.9rem;'>❌ Gemini AI: Not Configured</p></div>""", unsafe_allow_html=True)
        st.session_state.gemini_key = None
    
    st.markdown("---")
    st.markdown("### 📊 Data Source")
    sheet_url = st.text_input("Google Sheet URL", value="https://docs.google.com/spreadsheets/d/1j-l7KmbwK7h5Sp023pu2K91NOzVRQascjVwLLmtvPX4")
                ...
                if st.button(f"⏮️ Undo", key=f"undo_{idx}"):
                    with st.spinner("Reverting..."):
                        try:
                            ...
                            st.success("Reverted!"); time.sleep(1); st.rerun()
                        except Exception as e: st.error(f"Undo Failed: {e}")
    else: st.info("No save history")
...
with tab_verify:
    ...
    with col1: uploaded_file = st.file_uploader("Upload PDF, DOCX, Excel or Text file", type=['pdf', 'docx', 'txt', 'xlsx', 'xls'])
    with col2:
        f_name = uploaded_file.name if uploaded_file else (st.session_state.file_name if st.session_state.file_uploaded else "No file selected")
        st.markdown(f"""<div class="file-info-card"><div style="font-weight:700; margin-bottom:8px; color:#1E293B;">📄 Document Info</div><div style="font-size:0.9rem; color:#475569;"><strong>Name:</strong> {f_name}</div></div>""", unsafe_allow_html=True)
        ...
        with st.spinner("📂 Processing document..."):
            try:
                ...
                if st.session_state.spec_df is None:
                    pricing_df, addon_df, spec_df, def_dict = load_master_data(sheet_url)
                    st.session_state.pricing_df = pricing_df; st.session_state.addon_df = addon_df; st.session_state.spec_df = spec_df; st.session_state.def_dict = def_dict
                st.session_state.tor_raw_text = file_content; st.session_state.file_name = uploaded_file.name; st.session_state.file_size = uploaded_file.size; st.session_state.file_uploaded = True
                st.session_state.is_excel = uploaded_file.name.endswith(('.xlsx', '.xls'))
                st.rerun()
    ...
    if st.session_state.file_uploaded and not st.session_state.analysis_done:
        ...
        if st.button("🚀 Start AI Analysis", type="primary"):
            if not st.session_state.gemini_key: st.error("❌ Configure API Key first"); st.stop()
            progress_bar = st.progress(0); status_text = st.empty()
            try:
                status_text.markdown("**🤖 Step 1/4:** AI Structuring..."); progress_bar.progress(10)
                formatted_text = extract_scope_smart_ai(st.session_state.tor_raw_text, st.session_state.gemini_key) if enable_ai_formatting and not st.session_state.is_excel else st.session_state.tor_raw_text
                
                status_text.markdown("**📝 Step 2/4:** Extracting..."); progress_bar.progress(30)
                sentences = extract_sentences_from_tor(formatted_text)
                
                status_text.markdown("**🎯 Step 3/4:** Matching..."); progress_bar.progress(50)
                matched_products, result_df = analyze_tor_sentences_full_mode(sentences, st.session_state.spec_df, st.session_state.gemini_key)
                
                progress_bar.progress(80)
                if enable_fr_nfr:
                    status_text.markdown("**📊 Step 4/4:** Classifying...")
                    result_df['Requirement_Type'] = classify_scope_hybrid(sentences, st.session_state.gemini_key)
                else: result_df['Requirement_Type'] = 'Functional'
                
                progress_bar.progress(100)
                if '📝 Status' not in result_df.columns: result_df['📝 Status'] = '🤖 Auto'
                st.session_state.processed_df = result_df; st.session_state.matched_products = matched_products; st.session_state.analysis_done = True
                
                st.session_state.original_selections = {}
                for idx in result_df.index:
                    st.session_state.original_selections[idx] = {'products': str(result_df.loc[idx, 'Product_Match']), 'implementation': str(result_df.loc[idx, 'Implementation'])}
                st.rerun()
            except Exception as e: st.error(f"❌ Analysis failed: {e}")

    if st.session_state.analysis_done:
        st.markdown("### 📊 Analysis Results")
        # Ensure processed_df is the source of truth for stats
        df_stats = st.session_state.processed_df.copy()
        
        total_req = len(df_stats)
        
        # Use simple string checks as processed_df always has string columns
        ...
        with sc2:
            ...
            <div class="custom-card">
                ...
                <div class="stat-row"><span class="stat-name">Zocial Eye</span> <span class="stat-count">{cnt_ze}</span></div>
                <div class="stat-row"><span class="stat-name">Warroom</span> <span class="stat-count">{cnt_wr}</span></div>
                <div class="stat-row"><span class="stat-name">Outsource</span> <span class="stat-count">{cnt_out}</span></div>
                <div class="stat-row" style="border-bottom:none;"><span class="stat-name">Other</span> <span class="stat-count">{cnt_oth}</span></div>
            </div>""", unsafe_allow_html=True)
        with sc3:
            st.markdown(f"""
            <div class="custom-card">
                <div class="stat-label">Implementation Type</div>
                <div class="stat-row"><span class="stat-name">Standard</span> <span class="stat-count">{cnt_std}</span></div>
                <div class="stat-row" style="border-bottom:none;"><span class="stat-name">Customize</span> <span class="stat-count">{cnt_cust}</span></div>
            </div>""", unsafe_allow_html=True)
        ...
        with st.form("editor_form"):
            ...
            # 1. Define INTERNAL column names
            p_cols = {
                'Zocial Eye': 'P_Zocial Eye', 'Warroom': 'P_Warroom',
                'Outsource': 'P_Outsource', 'Other Product': 'P_Other Product',
                'Non-Compliant': 'P_Non-Compliant'
            }
            i_cols = {
                'Standard': 'I_Standard', 'Customize/Integration': 'I_Customize/Integration',
                'Non-Compliant': 'I_Non-Compliant'
            }

            # 2. Generate Checkbox Columns
            ...
            edited_df_input = st.data_editor(
                df, column_config=column_config, column_order=ordered_cols,
        ...
        if submit_changes:
            ...
            for i in working_df.index:
                ...
                # C. Reconstruct Strings
                curr_prods = get_checked_items(working_df.loc[i], p_cols)
                curr_impls = get_checked_items(working_df.loc[i], i_cols)
                ...
                def clean_set(s): return set([x.strip() for x in str(s).replace('[','').replace(']','').replace("'",'').split(',') if x.strip() and x.strip() != 'nan'])
                ...
                # Update Main State (Processed DF)
        ...
        # --- EXPORT (FIXED KEYERROR) ---
        st.markdown("### 💾 Export & Save")
        
        # ✅ FIX: Use processed_df directly (source of truth for strings)
        export_source = st.session_state.processed_df.copy()
        
        # Rename 'Product_Match' to 'Product' for consistency with downstream tools
        if 'Product_Match' in export_source.columns:
            export_source = export_source.rename(columns={'Product_Match': 'Product'})
            
        # Filter Non-Compliant
        valid_rows = export_source[~export_source['Product'].str.contains('Non-Compliant', na=False)].copy()
        ...
        if not valid_rows.empty:
            ...
            save_data = valid_rows[['Product', 'Sentence_TH', 'Sentence_ENG', 'Implementation']]
        else: save_data = pd.DataFrame(columns=['Product', 'Sentence_TH', 'Sentence_ENG', 'Implementation'])
        ...
        with c2:
            ...
            with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
                # Prepare Excel Data
                xl_data = st.session_state.processed_df.copy()
                cols = ['TOR_Sentence', 'Product_Match', 'Implementation', 'Requirement_Type']
                xl_data = xl_data[cols].rename(columns={'TOR_Sentence':'Requirement', 'Product_Match':'Selected product', 'Requirement_Type':'Requirement type'})
                xl_data.to_excel(writer, sheet_name='Data', index=False)
...
with tab_budget:
    ...
    if not st.session_state.analysis_done: st.warning("⚠️ Please complete verification first.")
    else:
...
st.markdown("---"); st.caption(f"WiseTOR Sense v2.5.3 | Session: {datetime.now().strftime('%Y-%m-%d')}")
